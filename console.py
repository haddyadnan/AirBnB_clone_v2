...
class HBNBCommand(cmd.Cmd):
    ...
    def __parse_args(self, args):
        """Allow for object creation with parameters"""
        args = args.strip().split()
        cls_name = args[0]
        params = {}
        for arg in args[1:]:
            item = arg.split("=")
            if item[1][0] == '"' and item[1][-1] == '"':  # Check String
                _pos = item[1].strip('"').find('"')
                if _pos != -1:

                    # before = item[1][: _pos - 1]
                    # print(before, "before")
                    value = item[1][: _pos + 1] + "\\" + item[1][_pos + 1 :]
                    print(item[1], value)
                    item[1] = value
    ...
    def do_create(self, args):
        ...
        cls_name, out = self.__parse_args(args)
        ...
        new_instance.attr_update(out)
        storage.save()
        print(new_instance.id)
        storage.save()

        # name, out = self.__parse_args(args)
        # print(out)
